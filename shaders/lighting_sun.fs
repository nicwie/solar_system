...
void main() {
    // Procedural Noise
    vec3 p = v_ModelSpacePos * 2.0;
    float noise = fbm_3D(p + u_time * 0.05) * 0.5 + fbm_3D(p - u_time * 0.02) * 0.5;

    // Color Mapping
    vec3 color1 = vec3(1.0, 0.3, 0.0); // Dark, fiery orange
    vec3 color2 = vec3(1.0, 0.8, 0.0); // Bright yellow/white hot
    ...
    float limb = smoothstep(0.0, 0.6, dot(normalize(Normal), viewDir));
    ...
    // Add a very bright, hot core based on the noise, also faded by the limb factor.
    finalColor += vec3(1.0, 1.0, 0.5) * pow(noise, 3.0) * limb;

    // The sun is emissive, not affected by external lighting.
